...
<body>
  <div class="container">
    <h2 style="margin:0 0 12px;">تعديل بيانات الطالب</h2>
    ...
    <!-- المهم: نستخدم $student (مفرد) ونرسل إلى students.update مع PATCH أو PUT -->
    <form method="POST" action="{{ route('students.update', $student->id) }}">
      ...
      <div class="field">
        ...
        <input type="date" name="course_date"
               value="{{ old('course_date', is_object($student->course_date) ? $student->course_date->format('Y-m-d') : $student->course_date) }}">
        @error('course_date') <div class="error">{{ $message }}</div> @enderror
      </div>

      <div class="field">
        <label>الدرجة</label>
        <input type="number" name="degree" value="{{ old('degree', $student->degree) }}" min="0" max="100">
